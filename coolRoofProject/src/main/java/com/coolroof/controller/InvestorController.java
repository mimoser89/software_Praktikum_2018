...
public class InvestorController {
	...
	public ModelAndView investorMyInvestment() {
		...
		for (Investment x : investments) {
			...
			totalSaved += UtilClass.Co2Calc.calcCo2(x.getInvestmentDate().getTime(), Integer.parseInt(x.getSpace()));
		}
		
		rtnOfInterest = totalSaved * constPaybackFactor;
		
		modelAndView.addObject("welcome", "Welcome, " + user.getName() + " " + user.getLastName());
		modelAndView.addObject("co2Saved", "CO2 saved in total: " + totalSaved + " Tonnen");
