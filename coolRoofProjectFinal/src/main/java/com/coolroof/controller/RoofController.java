...
public class RoofController {
	...
	@RequestMapping(value = "/login/addRoof", method = RequestMethod.GET)
	public ModelAndView addRoof() {
		...
		modelAndView.setViewName("login/addRoof");
		return modelAndView;
	}

	@RequestMapping(value = "/login/addRoof", method = RequestMethod.POST)
	public ModelAndView createNewRoof(@Valid Roof roof, BindingResult bindingResult) {
      ...
      // set price
			...
			modelAndView.addObject("successMessage", "Roof has been registered successfully");
			
			modelAndView.addObject("welcome", "Welcome " + user.getName() + " " + user.getLastName());
			modelAndView.addObject("roof", new Roof());
			modelAndView.setViewName("login/roofSuccess");
		}
		return modelAndView;
	}
	...
	@RequestMapping(value = "/login/owner_my_roofs", method = RequestMethod.GET)
	public ModelAndView myRoofs() {
		...
		for(Roof r : myroofs) {
			if (r.getTimeOfInvestment() == null)
				roofCo2.add(String.format("%.2f", 0.0));
			else {
				currentSaved = UtilClass.Co2Calc.calcCo2(r.getTimeOfInvestment().getTime(), (r.getArea() - r.getAreaLeft()));
		...
		modelAndView.setViewName("login/owner_my_roofs");
